#Создайте приложение и добавьте экран.
#Создайте интерфейс своего приложения (кнопки, метки и т.д.).
#Добавьте в интерфейс приложения возможность считывания файла.
#Реализуйте функцию OpenCV в интерфейсе.

import sys

import numpy as np
from PyQt5.QtWidgets import QApplication, QHBoxLayout, QMainWindow, QPushButton, QSpinBox,\
    QVBoxLayout, QWidget, QFileDialog, QErrorMessage, QLabel
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for start_val, prefix, spinbox in zip([3,12,2,15],['Threshold','R','G','B'],
                                              [self.threshold_select, self.r_select,
                                               self.g_select, self.b_select]):
    ...
    def choose_source_image(self):
        ...
        source_image_resized = resize_image(self.source_image_data, self.max_img_width,
                                            self.max_img_height)
...
def get_image_mask (img, blue_shade, diff):
    lower = np.array([max(0,val) for val in [blue_shade[2] - diff, blue_shade[1] - diff,
                                             blue_shade[0] - diff]], dtype=np.uint8)
    upper = np.array([min(0, val) for val in [blue_shade[2] + diff, blue_shade[1] + diff,
                                              blue_shade[0] + diff]], dtype=np.uint8)
